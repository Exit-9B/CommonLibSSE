...
namespace RE
{
	...
	class ActorValue : public BSTSingletonSDM<ActorValue>
	{
	public:
		enum Index
		{
			kNone = static_cast<std::underlying_type_t<Index>>(-1),
			kAggression = 0,
			kConfidence = 1,
			kEnergy = 2,
			kMorality = 3,
		...
		[[nodiscard]] static ActorValueInfo* GetActorValueInfo(Index a_index);

		[[nodiscard]] static const char* GetActorValueName(Index a_index);

		// members
		ActorValueInfo* actorValueInfos[Index::kTotal];  // 008
	};
	static_assert(offsetof(ActorValue, actorValueInfos) == 0x008);
